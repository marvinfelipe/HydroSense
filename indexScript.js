...
function fetchData() {
    // Generate random values for TDS, turbidity, pH, and temperature
    let tdsValue = getRandomValue(357.2, 357.2); // Random TDS between 0 and 2000 ppm
    if (tdsValue > 1500) {
        tdsValue = 1500;
    }

    let turbidityValue = getRandomValue(0.55, 0.56); // Random turbidity between 0 and 15 NTU
    if (turbidityValue > 10) {
        turbidityValue = 10;
    }

    let phValue = getRandomValue(5.34, 5.43); // Random pH between 0 and 14
    let temperatureValue = getRandomValue(30, 40); // Random temperature between 0 and 40 °C
    ...
    // Get health risks and recommendations based on the generated data
    const healthRisksText = getHealthRisks(tdsValue, turbidityValue, phValue);
    const recommendationText = getRecommendation(tdsValue, turbidityValue, phValue);
